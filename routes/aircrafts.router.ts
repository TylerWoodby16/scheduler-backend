...
aircraftsRouter.put("/:id", async (req: Request, res: Response) => {
    ...
    try {
        ...
        if(!collections.aircrafts) throw new Error();
        const result = await collections.aircrafts.updateOne(query, { $set: updatedAircraft });
...
aircraftsRouter.put("/updateMany/:name", async (req: Request, res: Response) => {
    ...
    try {
        ...
        // $set adds or updates all fields
        if(!collections.aircrafts) throw new Error();
        const result = await collections.aircrafts.updateMany(query, { $set: { year: update.year } });
